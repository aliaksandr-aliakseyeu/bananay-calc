...
def get_user_id_from_token(token: str) -> int:
    ...
    if subject_type in {"driver", "dc", "courier", "delivery_point"}:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def create_delivery_point_access_token(
    point_account_id: str,
    expires_delta: timedelta | None = None,
) -> str:
    """Create JWT access token for delivery point account (subject_type=delivery_point)."""
    data = {"sub": point_account_id, "subject_type": "delivery_point"}
    return create_access_token(data, expires_delta=expires_delta)


def create_delivery_point_refresh_token(point_account_id: str) -> str:
    """Create JWT refresh token for delivery point account."""
    to_encode = {
        "sub": point_account_id,
        "subject_type": "delivery_point",
        "type": "refresh",
    }
    now = datetime.now(timezone.utc)
    expire = now + timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS)
...
def get_delivery_point_id_from_token(token: str) -> str:
    """Extract delivery point account ID (UUID string) from JWT token."""
    payload = decode_token(token)
    if payload.get("subject_type") != "delivery_point":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Delivery point token required",
            headers={"WWW-Authenticate": "Bearer"},
        )
    point_id: str | None = payload.get("sub")
    if not point_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Could not validate credentials",
